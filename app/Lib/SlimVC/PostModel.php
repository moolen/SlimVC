...
class PostModel {
    ...
    /**
     * Returns the given WordPress post's title. If $strlen is given,
     * the title will be cut to the given length, depending
     * on the last whitespace. If $raw is TRUE, no WordPress filter
     * will be applied on the title.
     *
     * @author Michael Bindig <michael.bindig@sopg.de>
     *
     * @param WP_Post|int $post WordPress post object or ID
     * @param int $strlen (optional) The character-length of the returned string. Leave blank or set to NULL to get the whole title
     * @param boolean $raw (optional) Set to TRUE, if no WordPress filters shall be applied (Default: FALSE)
     * @param string $closure (optional) String, used at the end of shortened post titles (Default: '...')
     * 
     * @return string Post title.
     */
    public static function get_a_Title($post, $strlen = null, $raw = false, $closure = '...')
    {
        $postModel = new PostModel($post);
        $title    = $postModel->getTitle($strlen, $raw, $closure);
        unset($postModel);
    ...
    /**
     * Returns the value of the custom field with the given name 
     * from the given WordPress post object or ID.
     * If expected custom field has more than one value,
     * an numeric array will be returned. If no such custom field
     * exists, NULL will be returned.
     * 
     ...
     * @return array|string|null
     */
    public static function get_a_CustomField($post, $name)
    {
        $postModel = new PostModel($post);
        $cField   = $postModel->getCustomField($name);
        unset($postModel);
    ...
    /**
     * Returns the featured image URL of the given WordPress post.
     * 
     * @author Michael Bindig <michael.bindig@sopg.de>
     * 
     * @param WP_Post|int $post WordPress post object or ID
     ...
     * @return string
     */    
    public static function get_a_FeaturedImageUrl($post)
    {
        $postModel = new PostModel($post);
        $featuredImageUrl = $postModel->getFeaturedImageUrl();
        unset($postModel);
    ...
    /**
     * Returns the given WordPress post's date. If $format is given, post_date will be
     * returned formatted. Use the format syntax of PHP's date() function.
     *
     * @author Michael Bindig <michael.bindig@sopg.de>
     *
     ...
     * @return string Formatted or unformatted post_date
     */    
    public static function get_a_Date($post, $format = '')
    {
        $postModel = new PostModel($post);
        $date     = $postModel->getDate($format);
        unset($postModel);
    ...
    /**
     * Returns the whole custom field array 
     * of the given WordPress post.
     * 
     * @author Michael Bindig <michael.bindig@sopg.de>
     * 
     * @param type $post
     * @return type
     */
    public static function get_the_CustomFields($post)
    {
        $postModel = new PostModel($post);
        $cFields  = $postModel->cFields;
        unset($postModel);
    ...
    /**
     * Returns an array with all sub-posts 
     * of the given WordPress post, if there are any. 
     * Returns an empty array if no sub-posts can be found.
     *
     * @author Michael Bindig <michael.bindig@sopg.de>
     *
     * @param $post $post WordPress post object or ID
     * @param array $args (optional) Array with arguments, used by the
     *                    WordPress get_children() function (Default: NULL)
     *
     * @return array
     */    
    public static function get_the_subPosts($post, $args = null)
    {
        $postModel = new PostModel($post);
        $subPosts = $postModel->getSubPosts($args);
        unset($postModel);
    ...
    /**
     * Returns the postModel object, having the given slug,
     * or - if $wpPostObj is set to TRUE - the WordPress post object.
     *
     * @author Michael Bindig <michael.bindig@sopg.de>
     *
     * @global wpdb $wpdb
     * 
     * @param string $slug Slug of the WordPress post that shall be returned as postModel or WP_Post object
     * @param boolean $wpPostObj (optional) Set to TRUE to return a WP_Post instead of a postModel object
     * 
     * @return postModel|WP_Post
     */
    public static function get_a_PostBySlug($slug, $wpPostObj = false)
    {
        global $wpdb;
        ...
        $result = $wpdb->get_var(
            $wpdb->prepare($query, $wpdb->posts, $slug)
        );

        if (false !== $result && null !== $result)
        {
            $postModel = new PostModel((int) $result);
    ...
    /**
     * Returns TRUE, if given WordPress post
     * has sub-posts, otherwise FALSE.
     * 
     * @author Michael Bindig <michael.bindig@sopg.de>
     * 
     * @return boolean
     */    
    public static function has_PostSubPosts($post)
    {
        $postModel = new PostModel($post);
        $hasSubs  = $postModel->hasSubPosts();
        unset($postModel);
     ...
     * @return boolean
     */    
    public static function is_PostChildOf($post, $parent)
    {
        $postModel = new PostModel($post);
        $return   = $postModel->isChildOf($parent);
        unset($postModel);
    ...
    /**
     * Returns TRUE, if given WordPress post is connected 
     * to the WordPress category with the given slug,
     * otherwise FALSE will be returned.
     *
     * @author Michael Bindig <michael.bindig@sopg.de>
     ...
     * @return boolean
     */
    public static function is_PostInCategory($post, $slug)
    {
        $postModel = new PostModel($post);
        $return   = $postModel->isInCategory($slug);
        unset($postModel);
